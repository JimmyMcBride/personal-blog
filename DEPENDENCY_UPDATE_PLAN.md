...
| `@skeletonlabs/tw-plugin` | 0.1.0 | *(removed in v3)* | — | Remove |
...
| `remark-unwrap-images` | 4.0.0 | *(deprecated)* | — | Replace with `rehype-unwrap-images` |
...
1. **Update packages:**
   ```bash
   pnpm add -D svelte@^5 @sveltejs/vite-plugin-svelte@^7
   ```
...
Svelte 5 rejects malformed HTML (e.g. `<p><div>...</div></p>`). Run `pnpm check` to find these.
...
```bash
pnpm add -D @sveltejs/kit@latest
```
...
5. **Update Tailwind plugins:**
   ```bash
   pnpm add -D @tailwindcss/forms@^0.5.11 @tailwindcss/typography@^0.5.19
   ```
...
1. **Update packages:**
   ```bash
   pnpm add -D eslint@^10 typescript-eslint@^8 eslint-plugin-svelte@^3
...
```bash
pnpm add flowbite@^4 flowbite-svelte@^1 flowbite-svelte-icons@^3
```
...
```bash
pnpm add @tiptap/starter-kit@^3 svelte-tiptap@^3
```
...
```bash
pnpm remove remark-unwrap-images
pnpm add rehype-unwrap-images
```
...
```bash
pnpm add pocketbase@^0.26 dompurify@^3.3 sitemap@^9
pnpm add -D svelte-check@^4.4 typescript@^5.9 mdsvex@^0.12.7
```
...
|---|---|---|---|
| 1 | `svelte@5`, `@sveltejs/vite-plugin-svelte@7` | Medium-High | Use `npx sv migrate svelte-5`; incremental OK |
